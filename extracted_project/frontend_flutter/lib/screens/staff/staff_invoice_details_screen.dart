...
class _StaffInvoiceDetailsScreenState extends State<StaffInvoiceDetailsScreen> {
  ...
  List<Widget> _buildInvoiceTestList(List<dynamic> tests) {
    return tests.map((test) {
      ...
      final price = (test['price'] ?? 0).toDouble();
  ...
  Widget _buildPaymentSummary() {
    ...
    return Column(
      children: [
        // Invoice breakdown
        _buildSummaryRow('Subtotal', (totals['subtotal'] ?? 0).toDouble()),
        if ((totals['tax'] ?? 0) > 0)
          _buildSummaryRow('Tax', (totals['tax'] ?? 0).toDouble()),
        ...
        Builder(
          builder: (context) {
            final totalValue = (totals['total'] ?? 0).toDouble();
            // print(
