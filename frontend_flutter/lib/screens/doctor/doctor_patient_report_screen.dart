...
class _DoctorPatientReportScreenState extends State<DoctorPatientReportScreen> {
      ...
      if (mounted) {
        ...
        setState(() {
          _orderInfo = {
            ...
            'patient_id': response['patient']?['identity_number'],
            'patient_gender': response['patient']?['gender'],
            'patient_dob': response['patient']?['date_of_birth'],
            'patient_phone': response['patient']?['phone_number'],
            'patient_email': response['patient']?['email'],
            'patient_insurance': response['patient']?['insurance_provider'],
            'lab_name': response['lab']?['name'],
            'lab_address': response['lab']?['address'],
            'lab_phone': response['lab']?['phone'],
            'doctor_name': response['doctor_name'],
          };
          _results = results;
          _hasAbnormalResults = hasAbnormal;
          _abnormalCount = abnormalCnt;
          _isLoading = false;
        });
      }
    } catch (e) {
          ...
          return [
              ...
              pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.Text(
                      '$_abnormalCount abnormal value(s) detected. Please contact your healthcare provider immediately for interpretation and next steps.',
                      style: const pw.TextStyle(
                        fontSize: 12,
            ...
            pw.Table(
              ...
              children: [
                _buildPdfRow(
                  'Patient Name',
                  _orderInfo?['patient_name'] ?? 'N/A',
                ),
                _buildPdfRow('ID Number', _orderInfo?['patient_id'] ?? '-'),
                _buildPdfRow('Gender', _orderInfo?['patient_gender'] ?? '-'),
                _buildPdfRow(
                  'Date of Birth',
                  _orderInfo?['patient_dob'] ?? '-',
                ),
                _buildPdfRow('Phone', _orderInfo?['patient_phone'] ?? '-'),
                _buildPdfRow('Email', _orderInfo?['patient_email'] ?? '-'),
                _buildPdfRow(
                  'Insurance',
                  _orderInfo?['patient_insurance'] ?? 'None',
                ),
                _buildPdfRow(
                  'Order Date',
                    ...
                    return [
                      pw.TableRow(
                        children: [
                          ...
                          _buildPdfCell(
                            result['result_value']?.toString() ?? 'N/A',
                          ),
  ...
  String _buildStaffName(Map<String, dynamic>? staff) {
    if (staff == null) return 'Unknown Staff';
                        ...
                        Row(
                          children: [
                            Expanded(
                              child: _buildInfoRow(
                                'ID Number',
                                _orderInfo?['patient_id'] ?? '-',
                              ),
                            ),
                            Expanded(
                              child: _buildInfoRow(
                                'Gender',
                                _orderInfo?['patient_gender'] ?? '-',
                              ),
                            ),
                          ],
                        ),
